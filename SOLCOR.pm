...
sub solcorport($;$$) {
    ...
    foreach my $record (@cmp) {
		...
		my $spinoffadj = autoserve("SOLCOR,$$record{SYMBOL},SPINOFFADJ,$usedate,EXACT")||1; #add a spinoffadj to deal with the case where price adjustment and zero price spinoff happen on the same time. This adj is reflecting the part of price adjustment
		...
		if ($options =~ /^(NEXTDAY)$/) {
			...
			$$record{"NRIV".$baseccy} = $$record{"IV".$baseccy} - $$record{SHINV}*($niv / $fxniv) if $fxdiv;
